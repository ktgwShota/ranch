...
export async function createPoll(data: CreatePollData, env: { DB: D1Database }) {
  try {
    ...
    await db
      .prepare(`
      INSERT INTO polls (id, title, duration, endDateTime, createdBy, createdAt, isClosed, password, scheduleId)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `)
      .bind(
        pollId,
...
// PollにScheduleを紐付け
export async function linkScheduleToPoll(pollId: string, scheduleId: string, env: { DB: D1Database }) {
  const db = getDB(env);
  await db.prepare('UPDATE polls SET scheduleId = ? WHERE id = ?').bind(scheduleId, pollId).run();
  return { success: true, data: { id: pollId }, error: undefined };
}
