...
import { linkPollToSchedule } from '../../../services/db/schedule';
...
export async function POST(req: Request) {
  try {
    ...
    // Schedule IDが指定されている場合、Schedule側にもPoll IDを紐付ける
    if (body.scheduleId && result.data?.id) {
      await linkPollToSchedule(body.scheduleId, result.data.id, env);
    }
