...
import type { AvailabilityStatus, DateTimeItem, Response } from '../components/shared/types';
...
const normalizeAvailability = (
  availability: { [key: string]: AvailabilityStatus | null }
...
export function useScheduleResponse({ scheduleId, responses, allDateTimes }: UseScheduleResponseProps) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/schedules/${scheduleId}/respond`, {
        ...
        body: JSON.stringify({ respondentId, name: voterName.trim(), availability: myAvailability }),
      });
      if (!response.ok) {
  ...
  }, [voterName, isSubmitting, isEditing, scheduleId, respondentId, myAvailability, router]);
  ...
  const handleShowInputForm = useCallback((show: boolean) => {
    if (show && !isSubmitted) {
